...
class DashboardController extends Controller
{
    ...
    public function empAttendance(Request $request)
    {
        ...
        return view('hr.attendance', ['user' => $user, 'getEmpAtte' => $getEmpAtte, 'empid' => $empid]);
    }
    ...
    public function viewAttendance(Request $request)
    {
        // dd($request->all());
        try {
            $empDetail = Employee::where('is_deleted', 0)->where('emp_id', $request->emp_id)->select('emp_name', 'emp_id')->get()->first();
            // dd($empDetail);
            ...
            // dd($getEmpAtte->data);
            $data = $getEmpAtte->data;
            return view('hr.viewAttendance', ['empDetail' => $empDetail, 'getEmpAtte' => $getEmpAtte, 'data' => $data]);
        } catch (ModelNotFoundException $exception) {
            return back()->withError($exception->getMessage())->withInput();
        }
    }
    ...
    public function manualAttendanceSave(Request $request)
    {
        try {
            ...
            return redirect()->back()->with("success", "Insert Attendance is Now Add Successfully  !");
        } catch (ModelNotFoundException $exception) {
            return back()->withError($exception->getMessage())->withInput();
        }
    }
